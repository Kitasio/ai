...
<div class="h-screen bg-emerald-500">
	...
	<div class="flex flex-col justify-center gap-5 mt-10 max-w-6xl md:mx-auto mx-5">
		<input
			bind:value={prompt}
			type="text"
			class="rounded-xl p-2 bg-emerald-500 border-2 border-emerald-50"
		/>
		<div class="flex justify-between items-center">
			<input
				bind:value={max_tokens}
				type="number"
				class="rounded-xl p-2 bg-emerald-500 border-2 border-emerald-50 w-32"
			/>
			<div on:click={send} class="border-2 border-emerald-50 p-2 rounded-xl text-emerald-100">click</div>
		</div>
	</div>
